...
| Request handler | `handle_write` | `(action, *, obj=None, data=None, mutate)` | result of `mutate` | The general write handler: run `mutate` through the full bracket (authorize → snapshot → mutate → before_commit → commit → after_commit) and return its result. `handle_create`/`handle_update`/`handle_delete` delegate to it; call it from a custom write action instead of hand-rolling the bracket. Overriding it changes the lifecycle for every write on the view. Delegates to the self-free `fr.run_write_action`. |
