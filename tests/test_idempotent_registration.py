...
* Re-rename endpoints (``create`` → ``fooview_create`` → ``fooview_fooview_create``),
...
* Risk corrupting the first registration's class attributes (``cls.schema``,
  ``cls.creation_schema``, ``cls.update_schema``, ``cls.listing_param_schema``,
...
def test_class_attributes_are_not_corrupted_by_second_registration():
    """The schema/creation_schema/update_schema/listing_param_schema generated by
    ...
    # class name, so e.g. WidgetView.create.__name__ == "widgetview_create". A
