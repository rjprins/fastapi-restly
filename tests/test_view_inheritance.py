...
def test_inherit_model_and_schema(sync_db):
    ...
    with fr.open_session() as session:
        ...
        fetched = view.get_one(created.id)
        assert fetched.name == "Cog"

        items = view.get_many({}).objects
        assert len(items) == 1


# ---------------------------------------------------------------------------
# 2. business-verb override shared across multiple subclasses
# ---------------------------------------------------------------------------


def test_handler_override_shared_across_subclasses(sync_db):
    """
    A business-verb override on a base view applies to every subclass.
...
def test_super_chain_in_handler_override(sync_db):
    ...
    class LogBase(fr.RestView):
        ...
        def create(self, schema_obj):
            ...
            result = super().create(schema_obj)
...
def test_inherit_exclude_routes(sync_db):
    ...
    class ReadOnlyBase(fr.RestView):
        model = Entry
        schema = EntrySchema
        exclude_routes = ("create_endpoint", "update_endpoint", "delete_endpoint")
...
def test_inherit_soft_delete_via_delete_object(sync_db):
    ...
    with fr.open_session() as session:
        ...
        view.handle_delete(rec.id)

        # Still exists in database but is flagged deleted
        fetched = view.get_one(rec.id)
