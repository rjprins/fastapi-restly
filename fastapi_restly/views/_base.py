...
#: The five wire-tier route shells generated by RestView / AsyncRestView.
...
def _warn_on_misuse(view_cls: type[View]) -> None:
    ...
    # 1. Route-shell override where a business-verb override was likely meant.
    if is_crud_view:
        for shell in sorted(_SHELL_NAMES & own.keys()):
            verb = shell.removesuffix("_endpoint")
            warnings.warn(
                f"{name} overrides the route shell '{shell}' (the wire tier). "
                f"Override a shell only to change the HTTP contract. For "
    ...
    if not is_crud_view:
        ...
        if (
            ...
            warnings.warn(
                ...
                f"AsyncRestView generate list/create/get/update/delete from "
                f"`model` + `schema`; subclass one and override the bare verbs "
