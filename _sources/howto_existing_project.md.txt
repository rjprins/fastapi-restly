...
{class}`RestView <fastapi_restly.views.RestView>` provides CRUD endpoints for
...
You can also keep plain FastAPI routes beside a view for endpoints
...
{meth}`create <fastapi_restly.views.RestView.create>`, {meth}`update <fastapi_restly.views.RestView.update>`, {meth}`delete <fastapi_restly.views.RestView.delete>`); for a
...
{class}`RestView <fastapi_restly.views.RestView>`, map the existing HTTP
contract first:
...
{func}`fr.configure() <fastapi_restly.db.configure>` builds the session
factory on top. {class}`RestView <fastapi_restly.views.RestView>` commits
...
out). {class}`RestView <fastapi_restly.views.RestView>` commits its write actions.
...
{class}`RestView <fastapi_restly.views.RestView>` still owns the commit.
...
{class}`RestView <fastapi_restly.views.RestView>`:
...
{class}`RestView <fastapi_restly.views.RestView>` supports these models for
